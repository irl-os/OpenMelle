...
OpenMelle is a fork of OpenManus focused on persistent agent sessions and enhanced automation capabilities.
...
## Remote Desktop Access (VNC with Chrome Support)
...
```bash
sudo apt update
sudo apt install -y tigervnc-standalone-server xfce4 xfce4-terminal google-chrome-stable
```
...
1. **Recommended secure connection (SSH tunnel)**:
```bash
ssh -L 5901:localhost:5901 root@your-server-ip
```
Then connect VNC client to `localhost:5901`

2. **Direct connection (less secure)**:
Connect VNC client directly to `your-server-ip:5901`

### Using Chrome in VNC Session
- Launch from XFCE application menu under "Internet"
- Or run from terminal: `google-chrome`
- Supports full graphical browsing and extensions

### Troubleshooting
- If Chrome won't start, try running with:
```bash
...
OpenMelle is built by contributors from everywhere.
